...
| C-DAC VEGA AS4161 (RISC - V)   | [C-DAC VEGA AS4161 Configuration](https://github.com/binooa/GEMM-ArchProfiler/blob/main/cpuconf/CDAC_VEGAAS4161_RISC.py)   | The VEGA AS4161 is a 64-bit quad-core RISC-V processor developed by C-DAC (Centre for Development of Advanced Computing), India, as part of the country’s indigenous processor development initiative. Designed for high-performance and power-efficient computing, the AS4161 features a superscalar out-of-order pipeline compliant with the RV64IMAFD instruction set, enabling efficient execution of complex workloads. Each core includes a 32 KB L1 instruction cache and a 32 KB L1 data cache, along with integrated support for branch prediction, a Memory Management Unit (MMU), and a Vector Floating Point Unit (FPU). The processor supports virtual memory through TLB-based address translation and includes debug and interrupt support for embedded system integration. All cores are connected via a coherent interconnect and share a configurable L2 cache, facilitating scalable performance in multi-core environments. With standard interface compatibility (AHB/AXI4/ACE), the VEGA AS4161 is suitable for a range of embedded and general-purpose applications, showcasing India's capability in developing advanced RISC-V based processor architectures. |
